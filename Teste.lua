...
function Linux.Create(config)
    ...
    local totalTabs = tabOrder
    local totalItems = totalTabs + totalElements
    local baseLoadTime = 2
    local timePerItem = 0.1
    local totalLoadTime = baseLoadTime + (totalItems * timePerItem)
    ...
    spawn(function()
        ...
        spawn(function()
            local waveSpeed = totalLoadTime / 2
            for t = 0, totalLoadTime, 0.05 do
                for i, line in ipairs(waveLines) do
                    local offset = (i / #waveLines) * 2 * math.pi
                    local height = math.sin((t / waveSpeed) * 2 * math.pi + offset) * 15 + 10
                    line.Size = UDim2.new(0, 4, 0, height)
                end
                wait(0.05)
            end
        end)
        ...
        for i = 0, 100 do
            ProgressText.Text = "Loading... " .. tostring(i) .. "%"
            wait(totalLoadTime / 100)
        end

        local dissolve = TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        TweenService:Create(IntroFrame, dissolve, {BackgroundTransparency = 1}):Play()
        TweenService:Create(IntroTitle, dissolve, {TextTransparency = 1}):Play()
        TweenService:Create(WelcomeText, dissolve, {TextTransparency = 1}):Play()
        TweenService:Create(ProgressText, dissolve, {TextTransparency = 1}):Play()
        TweenService:Create(Glow, dissolve, {ImageTransparency = 1}):Play()

        for _, line in ipairs(waveLines) do
            TweenService:Create(line, dissolve, {BackgroundTransparency = 1}):Play()
        end

        for _, particle in pairs(ParticleContainer:GetChildren()) do
            if particle:IsA("Frame") then
                TweenService:Create(particle, dissolve, {BackgroundTransparency = 1}):Play()
            end
        end
