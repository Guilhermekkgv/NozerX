...
function Linux.Create(config)
    ...
    local IndicatorHolder = Linux.Instance("Frame", {
        Parent = TabsBar,
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 0),
        ZIndex = 3
    ...
    function LinuxLib.Tab(config)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            local targetY = tabPositionY - TabHolder.CanvasPosition.Y + (TabBtn.Size.Y.Offset - TabIndicator.Size.Y.Offset) / 2

            -- Definir os limites superior e inferior do TabsBar
            local tabsBarHeight = TabsBar.AbsoluteSize.Y
            local minY = 0
            local maxY = tabsBarHeight - TabIndicator.Size.Y.Offset

            -- Verificar se a aba selecionada tá visível dentro do TabsBar
            local tabTopY = tabPositionY - TabHolder.CanvasPosition.Y
            local tabBottomY = tabTopY + TabBtn.Size.Y.Offset
            local isTabVisible = (tabTopY + TabBtn.Size.Y.Offset >= 0) and (tabBottomY <= tabsBarHeight)

            -- Limitar o targetY aos limites do TabsBar
            targetY = math.clamp(targetY, minY, maxY)

            -- Ajustar a visibilidade do TabIndicator
            TabIndicator.Visible = isTabVisible

            isAnimating = true
            local tween = TweenService:Create(TabIndicator, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, 5, 0, targetY)})
        ...
        RunService:BindToRenderStep("Update", Enum.RenderPriority.Camera.Value + 1, function()
            if CurrentTab and Tabs[CurrentTab] and not isAnimating then
                ...
                local targetY = tabPositionY - TabHolder.CanvasPosition.Y + (tab.Button.Size.Y.Offset - TabIndicator.Size.Y.Offset) / 2
                ...
                -- Verificar se a aba selecionada tá visível dentro do TabsBar
                local tabTopY = tabPositionY - TabHolder.CanvasPosition.Y
                local tabBottomY = tabTopY + tab.Button.Size.Y.Offset
                local isTabVisible = (tabTopY + tab.Button.Size.Y.Offset >= 0) and (tabBottomY <= tabsBarHeight)
