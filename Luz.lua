...
function Linux.Create(config)
    ...
    -- Colocar o TabIndicator dentro do TabHolder pra acompanhar a rolagem
    ...
    local currentTween = nil -- Para controlar o Tween atual e evitar conflitos
    ...
    function LinuxLib.Tab(config)
        ...
        local TabText = Linux.Instance("TextLabel", {
            Parent = TabBtn,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, config.Icon and config.Icon.Enabled and -31 or -10, 1, 0),
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            -- Calcular a posição Y do TabIndicator dentro do TabHolder (sem subtrair CanvasPosition.Y)
            local tabPositionY = TabBtn.Position.Y.Offset + (tabIndex - 1) * (TabBtn.Size.Y.Offset + TabLayout.Padding.Offset) + TabPadding.PaddingTop.Offset
            local targetY = tabPositionY + (TabBtn.Size.Y.Offset - TabIndicator.Size.Y.Offset) / 2

            -- Cancelar qualquer animação anterior pra evitar conflitos
            ...
            isAnimating = true
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(TabIndicator, tweenInfo, {Position = UDim2.new(0, 5, 0, targetY)})
            currentTween = tween
        ...
        -- Função pra verificar se o TabIndicator tá dentro dos limites do TabsBar
        local function UpdateIndicatorVisibility()
            if not TabIndicator or not TabsBar or not CurrentTab or not Tabs[CurrentTab] then
                TabIndicator.Visible = false
                return
            end

            -- Pegar a posição absoluta do TabIndicator na tela
            local indicatorAbsPosY = TabIndicator.AbsolutePosition.Y
            local indicatorHeight = TabIndicator.AbsoluteSize.Y

            -- Pegar os limites do TabsBar
            local tabsBarAbsPosY = TabsBar.AbsolutePosition.Y
            local tabsBarHeight = TabsBar.AbsoluteSize.Y
            local tabsBarTop = tabsBarAbsPosY
            local tabsBarBottom = tabsBarAbsPosY + tabsBarHeight

            -- Verificar se o TabIndicator tá dentro dos limites do TabsBar
            local indicatorTop = indicatorAbsPosY
            local indicatorBottom = indicatorAbsPosY + indicatorHeight
            local isVisible = (indicatorBottom > tabsBarTop) and (indicatorTop < tabsBarBottom)

            TabIndicator.Visible = isVisible
        end

        -- Atualizar a posição do TabIndicator e a visibilidade durante a rolagem
        RunService:BindToRenderStep("Update", Enum.RenderPriority.Camera.Value + 1, function()
            if CurrentTab and Tabs[CurrentTab] and not isAnimating then
                local tab = Tabs[CurrentTab]
                if not tab.Button or not tab.Button.Parent then
                    -- A aba foi destruída, resetar o CurrentTab
                    CurrentTab = nil
                    TabIndicator.Visible = false
                    return
                end

                local tabIndex = CurrentTab
                local tabPositionY = tab.Button.Position.Y.Offset + (tabIndex - 1) * (tab.Button.Size.Y.Offset + TabLayout.Padding.Offset) + TabPadding.PaddingTop.Offset
                local targetY = tabPositionY + (tab.Button.Size.Y.Offset - TabIndicator.Size.Y.Offset) / 2

                TabIndicator.Position = UDim2.new(0, 5, 0, targetY)
            end

            -- Atualizar a visibilidade do TabIndicator
            UpdateIndicatorVisibility()
        end)

        -- Atualizar a visibilidade quando o TabHolder for rolado
        TabHolder:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
            UpdateIndicatorVisibility()
        end)

        -- Atualizar a visibilidade quando o tamanho ou posição do TabsBar mudar
        TabsBar:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            UpdateIndicatorVisibility()
        end)
        TabsBar:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
            UpdateIndicatorVisibility()
        end)

        -- Atualizar a visibilidade quando o tamanho do TabIndicator mudar
        TabIndicator:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            UpdateIndicatorVisibility()
        end)
