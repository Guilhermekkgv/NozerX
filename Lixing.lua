...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(TabIndicator, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, 0, 0, targetY)}):Play()
            CurrentTab = tabIndex
        end)
        ...
        if not CurrentTab then
            ...
            local tabPositionY = TabBtn.Position.Y.Offset + (tabIndex - 1) * (TabBtn.Size.Y.Offset + TabLayout.Padding.Offset) + TabPadding.PaddingTop.Offset
            local targetY = tabPositionY - TabHolder.CanvasPosition.Y + (TabBtn.Size.Y.Offset - TabIndicator.Size.Y.Offset) / 2
            TabIndicator.Position = UDim2.new(0, 0, 0, targetY)
            CurrentTab = tabIndex
        end
